...
}BTNode, * BTree, BinTreeNode; // BinTreeNode I used only for the function "createBinTreeByGList" now
...
void PostOrderNR(BTree T);// 后序遍历（非递归）
...
//后序遍历，非递归
//非递归后序遍历-test
void PostOrderNR(BTree T)
{
	BTree stack[20],p=NULL;
	char i, top = -1, loop = 1;
	for(i = 0; i < 20; i++)
        stack[i] = NULL;
	p = T;
	...
	while (top > -1)
	{
		...
		printf("%c ",p->data);
		if(top == -1)
            break;//if we have traversed the root we can just jump out,otherwise the next code will turn into a error.
		if(p == stack[top]->lchild || p == stack[top]->rchild)
            loop = 0;//loop is a sentinel var
		else 
            loop = 1;
		p = stack[top];
	}
